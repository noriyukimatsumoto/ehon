...
${story}`;
  ...
  async summarizeByTitle(title: string): Promise<string> {
...
- cover：物語「${title}」の表紙にふさわしいプロンプト。文字の記載がない、子ども向け絵本の表紙イラストを想定してください。登場人物や背景の特徴を具体的に描写してください。
- scenes：各シーンの内容を的確に表現する日本語プロンプト。filename は scene{N}.jpg（Nはシーン番号）。横長。登場人物を記載。背景や状況も具体的に描写。文字の記載はなし。子ども向け絵本のイラストを想定してください。
  - 以下の順でプロンプトを構成してください。
  ...
  - 例 scene1.jpg のプロンプト：「男の子が大きな心を抱えている様子。夜空の下、星が輝いている。男の子は笑顔で、周りには小さな動物たちが集まっている。全体的に温かみのある色合いで、子ども向け絵本のイラストスタイル。【登場人物】男の子・小さな動物たち」
${scenesText}`;
