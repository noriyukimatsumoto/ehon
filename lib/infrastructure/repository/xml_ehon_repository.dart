...
class XmlEhonRepository implements EhonRepository {
  ...
  Future<EhonData> fetchEhon(String xmlPath, String languageCode) async {
    ...
    final pages = document
        ...
        .map((n) => _parsePage(n, languageCode))
        .toList();
    final questions = document
        .findAllElements('question')
        .map((n) => _parseQuestion(n, languageCode))
        .toList();
  ...
  BookPage _parsePage(XmlElement node, String languageCode) {
    final pageDuration = int.tryParse(node.getAttribute('duration') ?? '');
    final textNodes = node.findElements('text').toList();
    final image = node.findElements('image').first.innerText.trim();
    ...
    return BookPage(texts: texts, image: image);
  }

  QuizQuestion _parseQuestion(XmlElement node, String languageCode) {
